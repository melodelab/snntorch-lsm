...
def long_short_Gabor_ensemble_lsm(
    in_conn, long_dist1=5, long_dist2=10, num_partitions=1
):
    ...
    conv_H = np.floor(
        1
        + (data.shape[-2] + 2 * padding - dilation * (filters.shape[-2] - 1) - 1)
        / stride
    )
    conv_W = np.floor(
        1
        + (data.shape[-1] + 2 * padding - dilation * (filters.shape[-1] - 1) - 1)
        / stride
    )
    ...
    inW = data.shape[-1]
    print(
        "possible connections per input neuron in receptive field: ",
        window * window * Nz,
    )
    print(
        "required connections per input neuron in receptive field: ",
        in_conn * Nx * Ny * Nz,
    )
    print(
        "#INFO: ensure that required connections is at most half of the possible connections (Note: Nz is scaled equally by partitions for both)"
    )

    Wins_s, Wlsm_s, conn_P1_s, _ = (
        initWeights_receptive_field_short_long_dist_partition(
            w_in,
            w_lsm,
            in_conn,
        ...
        for i, (data, targets) in enumerate(iter(trainloader)):
            ...
            if i == 0:
                ...
                in_train = np.concatenate(
                    (in_train, torch.mean(flat_data, dim=0).numpy()), axis=0
                )
        ...
        for i, (data, targets) in enumerate(iter(testloader)):
            ...
            if i == 0:
                ...
                in_test = np.concatenate(
                    (in_test, torch.mean(flat_data, dim=0).numpy()), axis=0
                )
